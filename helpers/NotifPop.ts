...
export class NotifPop{
  ...
  public add(notif:NOTIF_MESSAGE){
    ...
    //Container Style
    swapClass({id:notifElement, mk:"rounded w-96 bg-base-100 shadow-lg border relative pb-4 px-3"});

    //Close Button
    const closeButtonContainer = document.createElement("div");
    notifElement.appendChild(closeButtonContainer);
    closeButtonContainer.style.left = "98%";
    swapClass({id:closeButtonContainer, mk:"relative w-3 h-3 mr-2 mt-2 cursor-pointer"});
    closeButtonContainer.onclick = ()=>notifElement.remove();
    const closeButtonImage = document.createElement("img");
    closeButtonContainer.appendChild(closeButtonImage);
    closeButtonImage.src = "/images/interactive/close.svg";
