import { AnimeTroupe } from "./Animation";
import { popDispatch } from "./Modal";
/*|------------------------------------------------------------------------------------------|*/
/*|               Swapclass                                                                  |*/
/*|------------------------------------------------------------------------------------------|*/
interface Swapper{
...
export function DOMPopTransformer(instruction:{pop:string, [key:string|number]: any}){
	...
	const popClose = newPop.children[0].children[2];
	const popIcons = newPop.children[0].children[0];
	const popTitle = newPop.children[0].children[1];
	const popMessage = newPop.children[1];
	const popAdditional = newPop.children[2]
	const popButtonContainer = newPop.children[4];
...
function getFileLink(fileInput:HTMLInputElement, fileIndex = 0){
	if(!fileInput.files){
		return null;
	}
