...
export class Validation{
	...
	notRegex( argument: string|RegExp|RegExpConstructor|any[] ){
		this.validationStackPush((THIS)=>{
			const { input } = THIS.props;
			argument = anyToArr(argument);
			if(
	...
	public validate<T extends true|false>(raw=false):T extends false ? ValidationResult : Promise<string|true>{ //String only works on raw
		const THIS = this;
		const { action } = THIS;
		...
		if(raw){
			...
			}) as T extends false ? ValidationResult : Promise<string|true>
		}
		return new ValidationResult(iterateValidation()) as T extends false ? ValidationResult : Promise<string|true>;
