import { animate, AnimeTroupe } from "./Animation";
import { swapClass } from "./DOM";
...
export class ScreenPop{
  ...
  public breakPoint(whenToDown:{[key:number]:Partial<SCREENPOP_CONFIG>}){//Pixel Size
    const THIS = this;
  ...
  public run(){
    function applyConfig(canvasConfig:SCREENPOP_CONFIG, canvas:HTMLElement){
      ...
      if(canvasConfig.darken){
        ...
        if(canvasConfig.animate){
          setTimeout(()=>{
            const animateThis = new AnimeTroupe(canvas, [
      ...
      if(canvasConfig.highlightElements.length){
        const clipPoint = {
          head:[ ["0%", "0%"], ["0%", "100%"] ] as Array<Array<string>>,
          body:[ ] as Array<Array<string>>,
          tail:[ ["100%", "100%"], ["100%", "0%"] ]as Array<Array<string>>,
        }
        canvasConfig.highlightElements.forEach(element=>{
          ...
          //Body
          clipPoint.body.push([`${element.getBoundingClientRect().left}px`, `${element.getBoundingClientRect().top}px`]);
          clipPoint.body.push([`${element.getBoundingClientRect().left+element.offsetWidth}px`, `${element.getBoundingClientRect().top}px`]);
          clipPoint.body.push([`${element.getBoundingClientRect().left+element.offsetWidth}px`, `${element.offsetHeight+element.getBoundingClientRect().top}px`]);
          clipPoint.body.push([`${element.getBoundingClientRect().left}px`, `${element.offsetHeight+element.getBoundingClientRect().top}px`]);
      ...
      if((canvasConfig.popOver) && canvasConfig.popOver.length){
        canvasConfig.popOver.forEach((pop, i)=>{
          ...
          swapClass({id:popOverContainer, mk:"bg-base-200 rounded px-3 py-2 relative"});
          const minWidth = referenceElement.offsetWidth < 200 ? 200 : (referenceElement.offsetWidth < 400 ? referenceElement.offsetWidth : 400);
          ...
          swapClass({id:miniAnimation, mk:" animate-pulse flex justify-center mt-1"});
          const animationContainer = document.createElement("div");
          swapClass({id:animationContainer, mk:"relative w-full animate-bounce"});
          const textMessageForAnimation = document.createElement("div");
          textMessageForAnimation.textContent = "click to continue";
          swapClass({id:textMessageForAnimation, mk:"text-xs w-full text-center"});
          ...
          switch(position){
            case "top":
               ...
               //arterial
               popOverContainer.style.left = `${referenceElement.getBoundingClientRect().left + referenceElement.offsetWidth*.5}px`;
               ...
               pointerElement.style.left = `${actualWidth*.5}px`;
            ...
            case "right":
              ...
              //arterial
              popOverContainer.style.top = `${referenceElement.getBoundingClientRect().top + referenceElement.offsetHeight*.5}px`;
      ...
      if(canvasConfig.trigger != undefined){
        ...
        canvas.addEventListener("click",()=>{
          if(canvasConfig.trigger)
            canvasConfig.trigger(THIS as ScreenPop);
    ...
    function useBreakPoint(originalConfig:SCREENPOP_CONFIG, breakPointList:{[key: number]: SCREENPOP_CONFIG;}){
      ...
      const sortedWidthPoint = (Object.keys(breakPointList) as unknown as Array<keyof typeof breakPointList>).sort((a, b)=>a-b) as Array<keyof typeof breakPointList>;
