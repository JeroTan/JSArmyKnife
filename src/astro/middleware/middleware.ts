	...
	checkMatch() {
		...
		const result =
			this.pathList.length > 0
				? this.pathList.every((pathData) => {
						let result = false;
						//Check if path from pathname exist in pathData List
						if (pathData.allowProceeding) {
							//Allow proceeding means that the path can allow to be a subpath of the pathData
