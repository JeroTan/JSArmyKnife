	...
	public run(): [() => void, () => void, HTMLDivElement] {
		...
		function applyConfig(THIS: ToolTip): [() => void, () => void, HTMLDivElement] {
			...
			if (THIS.outside) {
				...
				switch (THIS.position) {
					case "top":
						outsideContainer.style.left = `${THIS.referenceElement.getBoundingClientRect().left + THIS.referenceElement.clientWidth * 0.5}px`;
						outsideContainer.style.top = `${THIS.referenceElement.getBoundingClientRect().top + THIS.referenceElement.clientHeight * 0.5}px`;
						break;
					case "right":
