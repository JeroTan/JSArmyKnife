...
export class Modal {
	...
	constructor(dispatch: (instruction: MODAL_INSTRUCTION) => void) {
		...
		this.frame = {
			//This will be the basis of types
			close: {
				...basicContent,
				isOpen: false,
			},
	...
	//--In-House Helper--//
	cacheData(object: Partial<MODAL_STRUCTURE>) {
		//Accepts object key:value
		const THIS = this;
		for (const key in object) {
			...
			(THIS.cachedContent as MODAL_STRUCTURE as any)[key as keyof MODAL_STRUCTURE] =
				object[key as keyof MODAL_STRUCTURE]!;
		}
		return THIS;
	}
	//--In-House Helper--//
	...
	//--Config--//
	type(type: "close" | "success" | "error" | "info" | "warning" | "loading" | "custom") {
		//This will determine the basic structure of the popup
	...
	callback(
		accept: ((close: Function) => void) | undefined = undefined,
		reject: ((close: Function) => void) | undefined = undefined,
		close: ((close: Function) => void) | undefined = undefined,
		backdrop: ((close: Function) => void) | undefined = undefined,
	) {
		this.cacheData({
