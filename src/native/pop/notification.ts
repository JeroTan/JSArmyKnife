...
export class NotifPop {
	...
	public add(notif: NOTIF_MESSAGE) {
		...
		if (template == null) {
			...
			swapClass({
				id: notifElement,
				mk: "rounded w-96 bg-gray-50 shadow-lg border border-gray-300 relative pb-4 px-3",
			});
			...
			swapClass({
				...
				mk: "relative size-3 mr-2 mt-2 cursor-pointer",
			});
		...
		const animation = new AnimeTroupe(notifElement, [{ opacity: "0" }, { opacity: "1" }], {
			duration: 200,
			easing: "easeOutQuad",
		});
		animation.play();
