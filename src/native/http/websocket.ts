...
export class WebSocketNative {
	...
	async urlCheck(callbackSuccess?: Function, callbackFail?: Function) {
		try {
			...
			console.log(
				...
				(this.url + this.param).replace("wss://", "https://"),
	...
	open(callback?: (e: Event) => void) {
		if (!this.url) {
			console.log(`URL is not in this web socket native instance`);
			return this;
		}
		...
		try {
			...
			if (callback) this.socket?.addEventListener("open", callback);
		} catch {
	...
	receiver(callback: (e: MessageEvent) => void) {
		if (this.socket == undefined || !this.url) {
			console.log(`${this.socket == undefined ? "Websocket" : "URL"} is not in this web socket native class`);
			return this;
		}
	...
	close(callback?: (e: Event) => void) {
		...
		this.socket.close();
		if (callback) this.socket.addEventListener("close", callback);
