...
export function animate(
	...
	time: ANIMATION_TIMER_OBJECT | KeyframeEffectOptions | string | number,
) {
...
export class AnimeTroupe {
	//It means as long as the configuration is the same you can redo the animation whenever you want for example a predefined time object.
	...
	constructor(
		element: KeyframeEffect | HTMLElement | any = null,
		keying: ANIMATION_KEYING_OBJECT[] | null = null,
		time: ANIMATION_TIMER_OBJECT | KeyframeEffectOptions | string | number | any = 0,
	) {
	...
	renew(
		...
		if (element) element = this.element(element).config.element;
		if (keying) keying = this.keying(keying).config.keying;
		if (time) time = this.time(time).config.time;
	...
	reset() {
		this.currentAnimation = new Animation(
			new KeyframeEffect(this.config.element, this.config.keying, this.config.time),
		);
		return this;
	}
	play(unChange = false) {
		if (!unChange) this.currentAnimation = this.reset().currentAnimation;
...
class TurnItem {
	...
	public do: (thisItem: TurnItem) => void = (thisItem: TurnItem) => {
		thisItem;
	};
}
...
class TurnOrder {
	...
	public addOrder(turnItem: TurnItem, sort = false) {
		...
		if (sort) this.stack.sort((a: TurnItem, b: TurnItem) => a.startTime - b.startTime);
		...
		while (index + 1 < this.stack.length) {
			if (this.stack[index].startTime === this.stack[index + 1].startTime) {
				//To ensure that it will be in the last of same startTime
				index++;
			} else {
				break;
			}
	...
	public removeOrder(id: string | number, sort = false) {
		if (this.stack.length <= 0)
			//if there is none to delete then quit
			return this;
		if (sort) this.stack.sort((a: TurnItem, b: TurnItem) => a.startTime - b.startTime);
...
export class TimelineFrame {
	...
	constructor(turnOrder?: TurnOrder | undefined, timeInterval?: number | undefined) {
		if (turnOrder) this.setTurnOrder(turnOrder);
	...
	public run() {
		this.timer = setInterval(() => {
			...
			const ms =
				this.runningMilliseconds + this.runningSeconds * Second + this.runningMinutes * Minute + this.timeInterval;

			while (1) {
				const item = this.turnOrder?.first();
				if (!item || item.startTime > ms) break;

				if (item.startTime <= ms) {
					item.do(item);
					this.turnOrder?.queuedOrder();
					if (item.repeat === null || item.repeat === undefined || item.repeat <= 0) continue;

					if (item.repeat !== Infinity) --item.repeat;
					++item.iterationCount;
			...
			function timeAdder(baseLine: number, currentTime: number, timeToAdd: number) {
				...
				if (currentTime < baseLine) return [currentTime, 0];
	...
	public currentTime(useMilliseconds = false) {
		if (useMilliseconds) return this.runningMilliseconds + this.runningSeconds * Second + this.runningMinutes * Minute;
		else return this.runningMilliseconds + this.runningSeconds * Second + this.runningMinutes * Minute; //Will change this in the future
	}
	public interrupt(turnOrder?: TurnOrder | undefined) {
		clearInterval(this.timer);
		if (turnOrder) this.turnOrder = turnOrder;
		return this;
	}
	public getTurnOrder() {
