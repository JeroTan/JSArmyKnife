...
export function randomizer(
	...
	allowDecimal: boolean | number = false,
) {
	const randData = Math.random();
	const minMaxRand: number = randData * (max - min) + min;

	if (minMaxRand % 1 == 0)
		//If there is no decimal then return immediately
		return minMaxRand;

	if (allowDecimal) {
		//If there is a decimal and arguments allowed it, then adjust decimal
		return adjustDecimal(minMaxRand, allowDecimal);
	} else {
		//If not then just remove the rest.
 ...
 */
export function randomizerExclude(
	min: number = 0,
	max: number = Number.MAX_SAFE_INTEGER,
	exclude: number[] = [],
	allowDecimal: boolean | number = false,
...
export function adjustDecimal(number: number, addPlaceValue: boolean | number = true): number {
	...
	if (precisionCheck !== placeValue && precisionCheck !== 0) {
		//If the two number decimals didn't match, then further adjustments is needed.
...
export function combineNumber(data: SPLIT_NUMBER, maintainString: boolean = false): string | number {
	...
	if (maintainString) return result;
...
export function fixedNumber(number: number, limit: number = 2, cutWhere: "left" | "right" = "right") {
	...
	switch (cutWhere) {
		...
		case "right":
			return Number(
				(raw.sign ? "-" : "") +
					String(raw.whole).substring(String(raw.whole).length > limit ? String(raw.whole).length - limit : 0),
			);
	}
}
...
export function numberAddComma(number: number) {
	...
	return (
		(splittedNumber.sign ? "-" : "") +
		splittedNumber.whole.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
		(splittedNumber.decimal !== 0 ? "." + splittedNumber.decimal : "")
	);
}
