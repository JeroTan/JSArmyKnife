		...
		prefixes: [
			{
				number: "460469",
				carrier:
					"various operators, including  CSL Mobile, Sun Mobile, 3 HK, PCCW Mobile, SmarTone, CMHK, China Unicom",
	...
	},
	{
		...
		prefixes: [
			...
			},
			{
				number: "7",
				carrier:
					"Various GSM and CDMA, including Aircel GSM, Airtel, BSNL GSM & CDMA, Idea, Loop Mobile GSM, MTS CDMA, Reliance GSM & CDMA, S-Tel GSM, Uninor GSM, Videocon GSM, Vodafone GSM.",
				note: null,
			},
			{
				number: "8",
			...
			},
			{
				number: "91",
				carrier:
					"Various GSM and CDMA, including MTS CDMA, BSNL GSM & CDMA, Uninor GSM, Loop Mobile GSM, S-Tel GSM etc.",
				note: null,
			},
			{
				number: "92",
	...
	},
	{
		...
		prefixes: [
			...
			},
			{
				number: "76",
				carrier:
					"(76-Abcdef) where A is 1/2/3/4/5 for Alfa and 0/6/7/8/9 for Touch, until 01–2011, only 76-6 & 76-7 are available",
				note: null,
			},
		],
	},
...
export function getCountryCode(code: string): COUNTRY_CODE_DATA {
	...
	if (index < 0) throw new Error("Country Code Data is Invalid");
	return countryCodeList[index];
}

export function splitMobileNumber(
	mobileNumber: string,
): COUNTRY_CODE_DATA & { number: string; numberWithZero: string } {
	...
	return {
		...countryCodeData,
		number: removeCountryFromMobileNumber,
		numberWithZero: "0" + removeCountryFromMobileNumber,
	};
}

function binarySearchIndex<T>(
	list: T[],
	callback: (MiddleElement: T, FirstElement: T, LastElement: T) => -1 | 0 | 1 | number,
	indexPoint = 0,
): number | -1 {
	//For the callback you must use x <= y where x is the thing you need to search and y is reference
	...
	if (direction === 0) return indexPoint + halfPoint;
	else if (direction !== 0 && list.length <= 1) return -1;

	if (direction < 0) return binarySearchIndex(list.slice(0, halfPoint + 1), callback, indexPoint + 0);
	else return binarySearchIndex(list.slice(halfPoint + 1), callback, indexPoint + (halfPoint + 1));
}
