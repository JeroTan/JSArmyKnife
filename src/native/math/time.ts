...
export function numberOfDays(
	month: string | NUMBER_OF_MONTHS,
	year = new Date().getFullYear(),
	date: undefined | string | number | Date = undefined,
): 30 | 31 | 29 | 28 | undefined {
...
export function monthName(number: NUMBER_OF_MONTHS, format: "short" | "long" = "short") {
	//Short or Long
	switch (number) {
		case 1:
...
export function timeToMilliseconds(timeString: string) {
	if (!timeString.match(/^\d+:\d{1,2}:\d{1,2}(:\d{1,4})?$/)) return 0;
...
export class DateNavigator extends Date {
	...
	//---- Time Zone ----//
	setTimezone(custom?: string | TIMEZONE_NAME, updateTimeFormat: boolean = false) {
		//Sample timezone, empty means browser timezone *be careful with vpn users
		if (custom == null) {
	...
	//Stabilizer of Date
	normalize(whatTime: string, type: "min" | "max" = "min") {
		//@whatTime = Year(2037 or 1970), Month(0 or 11), Day(0, 28,29,30,31), Hour(0, 23), Minute(0, 59), Seconds(0, 59), Milliseconds(0,999)
...
function adjustTimeToInterval(interval: number, date: Date) {
