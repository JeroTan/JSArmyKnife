...
export function swapClass(query: SWAPPER | SWAPPER[], recalculate: boolean = false) {
  ...
  */
	...
	function swap(
		...
		//dom re-calculate
		if (recalculate) element.offsetHeight;
	}
...
export interface HTMLElementsWithProps<T> extends HTMLElement {
	...
	slotData: (
		content:
			| (string | HTMLElement | HTMLElementsWithProps<any> | Node)
			| {
...
export function ElementMaker<T extends object>(
	componentName: string,
	callback: (element: HTMLElementsWithProps<T>) => void,
	noBuild = false,
) {
			...
			this.callStackRunner(() => {
				...
				if (typeof slots === "object") {
					for (let slot in slots) {
						...
						)[slot] as string | HTMLElement | HTMLElementsWithProps<any> | Node;
 ...
 * @returns
 */
export function specialComponentSelector<T extends HTMLElement>(
	selectorString: string,
	refElement: HTMLElement | Document = document,
): T {
...
export class LoadOrder {
	...
	push(callback: LOAD_CALLBACK, fallback?: LOAD_FALLBACK) {
		...
		if (fallback) this.fallbackStack[this.orderStack.length] = fallback;
	...
	cutIn(index: number, callback: LOAD_CALLBACK, fallback?: LOAD_FALLBACK) {
		...
		if (fallback) this.fallbackStack.splice(index, 0, fallback);
		this.hammerTrigger.click();
		return this;
	}
	...
	insert(index: number, callback: LOAD_CALLBACK, fallback?: LOAD_FALLBACK) {
		...
		if (fallback) this.fallbackStack[index] = fallback;
		this.hammerTrigger.click();
		return this;
	}
...
export function DOMPopTransformer(instruction: { pop: string; [key: string | number]: any }) {
	...
	const newPop = (
		(
			(document.getElementById("template-pop-component") as HTMLMetaElement).content as unknown as HTMLElement
	...
	//-- Animation --//
	const popAnimation = new AnimeTroupe(newPop, [{ opacity: "0" }, { opacity: "1" }], {
		duration: 300,
		easing: "easeOutBack",
	});
	...
	if (structure.acceptButton) {
		if (structure.acceptButtonText) acceptButton.textContent = structure.acceptButtonText;
		if (structure.acceptButtonCallback && typeof structure.acceptButtonCallback == "function") {
			(acceptButton as HTMLButtonElement).onclick = () => {
				structure.acceptButtonCallback!(closePop);
			};
		} else {
			(acceptButton as HTMLButtonElement).onclick = () => {
				closePopAnimate();
	...
	if (structure.rejectButton) {
		if (structure.rejectButtonText) rejectButton.textContent = structure.rejectButtonText;
		if (structure.rejectButtonCallback && typeof structure.rejectButtonCallback == "function") {
			(rejectButton as HTMLButtonElement).onclick = () => {
				structure.rejectButtonCallback!(closePop);
			};
		} else {
			(rejectButton as HTMLButtonElement).onclick = () => {
				closePopAnimate();
	...
	if (structure.closeButton) {
		if (structure.closeButtonCallback && typeof structure.closeButtonCallback == "function") {
			...
			(popClose.children[0] as HTMLDivElement).onclick = () => {
				closePopAnimate();
			};
		}
	} else {
	...
	if (structure.customDialog) {
		...
		if (popButtonContainer) newPop.insertBefore(toInsertElement as HTMLElement | Node, popButtonContainer);
		else newPop.appendChild(toInsertElement as HTMLElement | Node);
	}

	if (!structure.acceptButton && !structure.rejectButton && !false) {
		//In case we can opt to have custom buttons
...
export class Prefetch {
	...
	fetch() {
		function frameLoad(frameData: { link: string; fetching: boolean; frameId: string }) {
	...
	swapFetched(success?: Function, fail?: Function) {
		function swap(frameData: { link: string; fetching: boolean; frameId: string }) {
			let index: number = 0;
...
export class Observer {
	...
	public observe(
		observeCallback: (mutations: MutationRecord[], observer: MutationObserver, thisClass: Observer) => void,
	...
	const observer = new IntersectionObserver(
		(entries) => {
			...
			entries.forEach((entry) => {
				...
				if (entry.boundingClientRect.top < documentHeight + offset && entry.boundingClientRect.bottom > 0) {
			...
			if (isIntersecting === intersectingComputation) return;
