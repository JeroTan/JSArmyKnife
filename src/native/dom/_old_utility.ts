...
export class FieldBinding {
	...
	private addField(
		name: string | number,
		offsetName: string,
		fieldList: { [key: string | number]: INPUT_FIELD },
		field?: INPUT_FIELD | string | undefined,
	) {
		fieldList[name] =
			field !== undefined && typeof field !== "string"
				? field
				: (document.getElementById(
						typeof field === "string" && field !== undefined ? field : offsetName + name,
					) as INPUT_FIELD);
		return this;
	}
	private listenTrigger(
		field: INPUT_FIELD | string | number,
		offsetName: string,
		triggerUpdate = false,
		callback?: Function,
	) {
		field =
			typeof field === "string" || typeof field === "number"
				? (document.getElementById(offsetName + String(field)) as INPUT_FIELD)
				: field;
		...
		field.addEventListener("input", (e) => {
			//Trigger this when input change.
			inputIsTriggered = true;
			changeIsTriggered = false;
			if (callback) callback(e);
			if (triggerUpdate) THIS.listenerElement.click();
		});
	...
	private removeField(
		...
		oldField: { [key: string | number]: INPUT_FIELD },
	) {
		const THIS = this;
	...
	private getValue(
		list: { [key: string | number]: INPUT_FIELD },
		name: string | number | undefined | Array<string | number> = undefined,
		...
		if (typeof name === "string" || typeof name === "number") {
			//Return only the string value if not
			if (!list[name] || list[name].value === null) return "";
	...
	private setValue(
		field: { [key: string | number]: INPUT_FIELD },
		name: string | number | Array<string | number> | { [name: string | number]: string },
		value?: string,
	...
	public addInputField(
		...
		field?: INPUT_FIELD | string | undefined | Array<INPUT_FIELD | string | undefined>,
		includeError = false,
	) {
		//Error will only get the names not the field so be careful
	...
	public getInput(name: string | number | undefined | Array<string | number> = undefined) {
		//If not defined then return all, if define by string return one, if define by array return that array
		return this.getValue(this.inputDOM, name);
	}

	public getError(name?: string | number | undefined | Array<string | number>) {
		//same with value but error
	...
	public validation(listToValidate: {
	...
	public listen(
		callback: (input: { [key: string | number]: string }, error: { [key: string | number]: string }) => void,
	) {
		this.listenersStack.push(callback);
	}

	public listenToInput(
		whatField: Array<string | number>,
		callback: (input: { [key: string | number]: string }, error: { [key: string | number]: string }) => void,
		returnAll = false,
	) {
		const THIS = this;
		...
		this.listen((input, error) => {
			...
			if (isChange)
				callback(
					returnAll ? input : (THIS.getInput(whatField) as { [key: string | number]: string }),
					returnAll ? error : (THIS.getError(whatField) as { [key: string | number]: string }),
				);
		});
	}
	public listenToError(
		whatField: Array<string | number>,
		callback: (input: { [key: string | number]: string }, error: { [key: string | number]: string }) => void,
		returnAll = false,
	) {
	...
	public listenTo(
		inputField: Array<string | number>,
		errorField: Array<string | number> | true,
		callback: (input: { [key: string | number]: string }, error: { [key: string | number]: string }) => void,
		returnAll = false,
	) {
		const THIS = this;
		...
		this.listen((input, error) => {
			...
			if (isChange)
				callback(
					returnAll ? input : (THIS.getInput(inputField) as { [key: string | number]: string }),
					returnAll
						? error
						: (THIS.getError(errorField === true ? inputField : errorField) as {
	...
	public onBlur(
		inputField: Array<string | number>,
		errorField: Array<string | number>,
		callback: (input: { [key: string | number]: string }, error: { [key: string | number]: string }) => void,
	) {
		const THIS = this;
		...
		piston.addEventListener("click", () => {
			callback(
				THIS.getInput(inputField) as { [key: string | number]: string },
				THIS.getError(errorField) as { [key: string | number]: string },
			);
		});
	}
	...
	public checkEmptyField(
		name: string | number | undefined | Array<string | number> = undefined,
		combine = false,
	): boolean | { [key: string | number]: boolean } {
		if (typeof name == "string" || typeof name == "number") {
		...
		if (name == undefined) {
			...
			(Object.keys(inputData) as Array<string>).forEach((x) => {
				resultObject[x] = inputData[x] !== "";
			});
			return resultObject;
		}
		if (combine) {
