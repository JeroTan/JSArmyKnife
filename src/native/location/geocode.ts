export function geocodeCheckIfInside(geocode:{lat:number, lng:number}, polygon:{lat:number, lng:number}[]){
  ...
  let isInside = false;
  let minX = polygon[0].lng, maxX = polygon[0].lng;
  let minY = polygon[0].lat, maxY = polygon[0].lat;
  ...
  for (let i = 0; i < polygon.length; i++) {
    if ((polygon[i].lat > geocode.lat) !== (polygon[j].lat > geocode.lat) &&
        geocode.lng < (polygon[j].lng - polygon[i].lng) * (geocode.lat - polygon[i].lat) / (polygon[j].lat - polygon[i].lat) + polygon[i].lng) {
      isInside = !isInside;
    }
    j = i;
  }
...
export function convertBoundTo2DPolygon(bound:{
  west: number,   
  north: number, 
  east: number,
  south: number, 
