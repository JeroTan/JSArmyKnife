...
export const validationErrorMessage: VALIDATION_ERROR_MESSAGE = {
	//feel free to extend this one too and change the content of errorMessages in Validation Class;
...
export class Validation {
	 ...
	 */
	number(argument: boolean = false) {
		//True means convert the string into number if it is a string with a number
		this.validationStackPush((THIS) => {
			const { input } = THIS.props;
		...
		this.validationStackPush((THIS) => {
			...
			if (typeof input === "string" || typeof input === "number") {
				...
				if (isNaN(Number(date))) {
					if (argument) THIS.props.input = date;
					return true;
				}
			}
	...
	same(argument: this) {
		//accept argument as instance of this class;
		this.validationStackPush((THIS) => {
			const { input } = THIS.props;
	...
	match(argument: Array<string | number> | string | number) {
	...
	custom(
		argument: (
			...
			| true
			| { validator: string; args?: Array<string | number> }
			| Promise<true | { validator: string; args?: Array<string | number> }>,
	) {
		this.validationStackPush((THIS) => {
	...
	public validate(raw = false): ValidationResult | Promise<string | true> {
		//String only works on raw
		const THIS = this;
		const { action } = THIS;
		...
		async function iterateValidation(validator: VALIDATION_RESPONSE[] = action, current = 0) {
			if (action.length <= current) {
				//It means that it reaches the validation stack limit and there is no error found.
				return true;
			}
			...
			if (result === true) {
				...
				let errorMessage: string = THIS.errorMessages[result.validator].replace(
					"%field%",
					display ? display : capitalFirst(field),
				);
				if (result.args) {
...
export class ValidationResult {
	...
	constructor(validationPromise: Promise<string | true | undefined> | undefined = undefined) {
		if (validationPromise !== undefined) this.addPromise(validationPromise);
	}
...
export function validationFactory<Type>(
	total: number | (string | number)[],
	ValidationClass: new (...args: any[]) => Type,
	...args: any
): ValidationFactoryList<Type> {
	//total string array is for FieldName
	const result: ValidationFactoryList<Type> = {};

	if (typeof total === "number") total = [...Array(total)];
...
export async function validationRunner<Type>(
	validatorList: ValidationFactoryList<Type>,
	resultOnly = false,
): Promise<ValidationRunnerList> {
	const result: ValidationRunnerList = {};
	for (const i in validatorList) {
		const ValConstruct = validatorList[i] as {
			validate: <T extends true | false>(raw?: boolean) => T extends false ? ValidationResult : Promise<string | true>;
		};
