...
export class Kizuna<RETURN_TYPE, SHARED_DATA extends object> {
	...
	private chainStack: Array<
		((sharedValues: SHARED_DATA) => Promise<void | RETURN_TYPE>) | ((sharedValues: SHARED_DATA) => void | RETURN_TYPE)
	> = [];
	...
	public link(
		callback:
			| ((sharedValues: SHARED_DATA) => Promise<void | RETURN_TYPE>)
			| ((sharedValues: SHARED_DATA) => void | RETURN_TYPE),
	) {
	...
	public async run() {
		if (this.chainStack.length < 1) return this.returnValue;
